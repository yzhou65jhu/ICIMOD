...
#This script is the third supplementary script of the disaggregation step (see 3.DisAgg.sh for reference). This script 1) adds metadata infomation to newly generted met forcing files, and 2) split one file into monthly files that LIS reads. 

#Set initial date of forecast and ensemble number. 
i=1
yyyy=2018
mmmdd=oct28
...
#Set units, coordinates boundary.
...
#Rename lat and lon variable names
echo "  renaming ens$n"
...
#Split one file into monthly files
cd $GEOS/middle/ens$n
mkdir -p $GEOS/ens$n
echo "  spliting ens$n"
...
#Change .nc to .nc4 which LIS recognize
cd $GEOS/ens$n/
